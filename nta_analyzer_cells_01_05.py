...
- Cell 05: Core calculations with uncertainty propagation
    ...
    for scale in df[scale_column].unique():
        ...
        if not scale_df.empty:
            ...
            if 'volume_nm^3_per_mL_avg' in scale_df.columns:
                ...
                scale_metrics['total_volume_um^3_per_mL_avg'] = total_volume_avg / 1e9  # nm³ to µm³
                scale_metrics['total_volume_uL_per_mL_avg'] = total_volume_avg / 1e18  # nm³ to µL
            ...
            if 'area_nm^2_per_mL_avg' in scale_df.columns:
                ...
                scale_metrics['total_surface_area_um^2_per_mL_avg'] = total_area_avg / 1e6  # nm² to µm²
...
class NTAAnalyzer:
    """
    Main NTA analysis class integrating all cells 01-05.
    ...
    def process(self, filepaths):
        """Process NTA files with full workflow (Cells 02-05)."""
        ...
        # Step 1: Apply dilution correction
        ...
        try:
            ...
            if success:
                ...
                processed_dist = dilution_corrected_df
            else:
        ...
        # Step 2: Normalize distributions
        ...
        # Step 3: Calculate cumulative distributions
        print("\n3. Calculating cumulative distributions with uncertainty...")
        try:
            final_df = calculate_cumulative_distributions_with_uncertainty(processed_dist)
            print("✓ Cumulative distributions completed successfully")
            processed_dist = final_df
        except Exception as e:
        ...
        try:
            ...
            print("✓ Total metrics calculation completed successfully")
            
            # Display metrics
            for scale, metrics_dict in total_metrics.items():
                if metrics_dict:
                    print(f"\n{scale.upper()} SCALE - KEY METRICS:")
                    if 'total_particles_per_mL_avg' in metrics_dict:
                        avg_val = metrics_dict['total_particles_per_mL_avg']
                        sd_val = metrics_dict.get('total_particles_per_mL_sd', 0)
                        print(f"  Particles/mL: {avg_val:.2E} ± {sd_val:.2E}")
                    if 'total_volume_uL_per_mL_avg' in metrics_dict:
                        avg_val = metrics_dict['total_volume_uL_per_mL_avg']
                        sd_val = metrics_dict.get('total_volume_uL_per_mL_sd', 0)
                        print(f"  Volume/mL: {avg_val:.4E} ± {sd_val:.4E} µL")
                    if 'total_surface_area_cm^2_per_mL_avg' in metrics_dict:
                        avg_val = metrics_dict['total_surface_area_cm^2_per_mL_avg']
                        sd_val = metrics_dict.get('total_surface_area_cm^2_per_mL_sd', 0)
                        print(f"  Surface area/mL: {avg_val:.4E} ± {sd_val:.4E} cm²")
        except Exception as e:
            print(f"✗ Total metrics calculation failed: {e}")
            total_metrics = {}
        
        # Step 5: Add metrics to metadata
        print("\n5. Adding calculated metrics to metadata...")
        try:
            metadata = add_metrics_to_metadata_with_uncertainty(metadata, total_metrics, scale='linear')
            print("✓ Metadata updated with calculated metrics")
        except Exception as e:
    ...
    def save_outputs(self, output_dir):
        ...
        Outputs:
        - Metadata files (with and without headers)
        - Comprehensive PSD file (both scales, all calculated columns)
        """
        ...
        # ===== Save Comprehensive PSD File (all data in one file) =====
        dist = self.results['distribution']
        
        if not dist.empty:
            psd_path = os.path.join(output_dir, f'Data_{unique_id}_PSD_COMPREHENSIVE.txt')
            
            # Keep scale column for proper identification of linear vs logarithmic data
            psd_export = dist.copy()
            
            # Save with tab separation, headers included
            psd_export.to_csv(psd_path, sep='\t', index=False)
            
            created_files.append(psd_path)
            print(f"✓ Saved comprehensive PSD file: {os.path.basename(psd_path)}")
            print(f"  Columns: {len(psd_export.columns)}")
            for scale in psd_export['scale'].unique():
                scale_rows = len(psd_export[psd_export['scale'] == scale])
                print(f"    {scale}: {scale_rows} rows")
